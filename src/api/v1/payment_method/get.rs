...
pub fn main() -> impl Filter<Extract = (impl warp::Reply,), Error = warp::Rejection> + Clone {
    warp::path("get")
        ...
        .and_then(move |token: String, user_id: i32, client_type: Option<String>| {
            async move {
                let access_token = model::RequestToken{
                    user_id,
                    token,
                };
                let if_token_valid_result = methods::tokens::verify_user_token(access_token.user_id.clone(), access_token.token.clone()).await;
