                ...
                return match if_token_valid {
                    ...
                    Ok(token_bool) => {
                        if token_bool {
                            ...
                            if !card_in_db {
                                let new_pm = stripe_veygo::create_new_payment_method(request_body.pm_id.as_str(), request_body.md5.clone(), request_body.cardholder_name.clone(), request_body.user_id.clone(), request_body.nickname).await;
                                match new_pm {
                                    Err(_) => {
                                    ...
                                    Ok(new_pm) => {
                                        ...
                                        if attach_result.status != SetupIntentStatus::Succeeded {
                                            let error_msg = serde_json::json!({"token": &request_body.token, "error": "PaymentMethods declined"});
