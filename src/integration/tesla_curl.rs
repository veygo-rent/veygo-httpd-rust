...
pub fn get_valid_bearer() -> Option<String> {
    get_tesla_token().and_then(|t| if !t.is_expired() { Some(t.bearer()) } else { None })
}
...
pub async fn fetch_and_store_tesla_token() -> Result<TeslaToken> {
    ...
    let audience = env_or_default("AUDIENCE", DEFAULT_AUDIENCE);
    let auth_url = env_or_default("AUTH_URL", DEFAULT_AUTH_URL);
    ...
    if !resp.status().is_success() {
        ...
        return Err(anyhow!("Tesla auth failed: status={} body={}", status, body));
    }
...
pub async fn tesla_make_request(method: reqwest::Method, path: &str, body: Option<String>) -> Result<reqwest::Response> {
    let client = reqwest::Client::new();
